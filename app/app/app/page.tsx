...
/**
 * Regulina-T™ | RGN-T1™ IMMUNOREGULATOR — Single-Page Portfolio
 * - Languages: EN / RU / AR (AR → RTL)
 * - Hero: premium hierarchy; CTA order Contact | Licensee (no dossier in hero)
 * - Header: compact inline badge sized like prior gray label
 * - Cards: mint background, border, soft shadow per spec
 * - Mailto: localized subjects/bodies; recipient official.regulina.t@gmail.com
 * - Dossier: language-specific download (/files/Regulina-T_License_(en|ru|ar).pdf)
 */
...
const dict = {
  ...
  RU: {
    ...
    blocks: {
      ...
      footer: "© Regulina-Т™ Платформа иммунорегуляции тимуса",
...
function buildContactMailto(lang: keyof typeof dict) {
  ...
  const bodyMap = {
    ...
    RU:
      "Уважаемая команда Regulina-T,%0D%0A%0D%0AМеня зовут (ФИО), я обращаюсь по вопросу сотрудничества с вашей компанией.%0D%0A%0D%0AКомпания/Организация: ___%0D%0AСтрана: ___%0D%0AТелефон: ___%0D%0AЭлектронная почта: ___%0D%0A%0D%0AПрошу связаться со мной для обсуждения деталей.%0D%0A%0D%0AС уважением,%0D%0A_______",
    AR:
...
function runSelfTests() {
  try {
    (["EN", "RU", "AR"] as const).forEach((k) => {
      ...
      console.assert(
        h.pretitle && h.title1 && h.title2 && h.badge === HEADER_BADGE,
        `${k} hero pieces missing / badge not constant`
      );
    });
    console.assert(
      !!dict.EN.blocks.positioningCard &&
        !!dict.RU.blocks.positioningCard &&
        !!dict.AR.blocks.positioningCard,
      "positioningCard missing"
    );
    const want = {
      autoimmune: "#1E88E5",
      aging: "#0CA678",
    ...
    marketPie.forEach((s) =>
      console.assert(
        s.color.toUpperCase() ===
          want[s.key as keyof typeof want].toUpperCase(),
        `Color mismatch for ${s.key}`
      )
    );

    // Mailto sanity
    const licEN = buildLicenseeMailto("EN");
    console.assert(
      licEN.startsWith(`mailto:${CONTACT_EMAIL}`),
      "Licensee recipient mismatch"
    );
    console.assert(
      licEN.includes("License%20application%20for%20Regulina-T"),
      "EN licensee subject missing"
    );

    const conRU = buildContactMailto("RU");
    console.assert(
      conRU.includes(encodeURIComponent("Запрос на сотрудничество")),
      "RU contact subject missing"
    );
    console.assert(
      conRU.startsWith(`mailto:${CONTACT_EMAIL}`),
      "Contact recipient mismatch"
    );

    // Dossier paths
    console.assert(
      getDossierPath("EN").endsWith("(en).pdf"),
      "Dossier path EN mismatch"
    );
    console.assert(
      getDossierPath("RU").endsWith("(ru).pdf"),
      "Dossier path RU mismatch"
    );
    console.assert(
      getDossierPath("AR").endsWith("(ar).pdf"),
      "Dossier path AR mismatch"
    );
...
function Card({
  children, className = "", clickable = false
}: { children: React.ReactNode; className?: string; clickable?: boolean }) {
  return (
    <div
        ...
        <div className="mx-auto grid w-full max-w-[1200px] grid-cols-1 items-center gap-10 px-4 py-12 md:grid-cols-2 md:py-20">
          ...
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.1 }}>
            <Card className="relative aspect-video w-full overflow-hidden p-0">
              ...
              <button
                onClick={() => alert("Embed your 1-minute video here (replace <video> source).")}
                className="absolute left-1/2 top-1/2 z-10 -translate-x-1/2 -translate-y-1/2 rounded-full bg-emerald-600 p-3 text-white"
                aria-label="Play"
              >
                <Play />
              </button>
            ...
            <Glow>
              <Card className="p-6 md:p-8">
                ...
                {d.blocks.scienceMain.map((p, i) => (
                  <p key={i} className="text-[#111827]">{p}</p>
                ))}
        ...
        >
          <div className={`flex ${isRTL ? "flex-row-reverse" : ""} flex-wrap items-center justify-between gap-3 p-3`}>
            ...
            <div className={`flex ${isRTL ? "flex-row-reverse" : ""} items-center gap-2`}>
              <a href={contactMailto} className="rounded-[14px] px-3 py-1.5 text-xs font-semibold text-white" style={{ background: "#16A34A" }}>
              ...
              <a href={dossierUrl} download className="rounded-[14px] border px-3 py-1.5 text-xs font-semibold text-slate-900" style={{ background: "#FFFFFF", borderColor: "#E2E8F0" }}>
